...
namespace QLS.DAO
{
    internal class CaiDatDAO
    {
        ...
        {
            DataProvider.Instance.ExecuteNonQuery("UPDATE CAIDAT SET GIAO_DIEN = '" + giaodien + "' WHERE TAI_KHOAN = '" + tk + "'");
        }
        public void CapNhatNgonNgu(string tk, int ngonngu)
        {
            DataProvider.Instance.ExecuteNonQuery("UPDATE CAIDAT SET NGON_NGU = '" + ngonngu + "' WHERE TAI_KHOAN = '" + tk + "'");
        }
        public void CapNhatTabMenu(string tk, int tabmenu)
        {
            DataProvider.Instance.ExecuteNonQuery("UPDATE CAIDAT SET MENU_TAB = '" + tabmenu + "' WHERE TAI_KHOAN = '" + tk + "'");
        }
        public void CapNhatColorApp(string tk, int colorapp)
        {
            DataProvider.Instance.ExecuteNonQuery("UPDATE CAIDAT SET COLOR_APP = '" + colorapp + "' WHERE TAI_KHOAN = '" + tk + "'");
        }
    }
}
