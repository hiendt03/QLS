...
namespace QLS
{
    public partial class form_XoaTaiKhoanNguoiDung : Form
    {
        ...
        private void button_DongY_Click(object sender, EventArgs e)
        {
            if (listtk == null)
            {
                AccountDAO.Instance.XoaNguoiDung(row.Cells[0].Value.ToString());
            }
            else
            {
                foreach (string tk in listtk)
                {
                    AccountDAO.Instance.XoaNguoiDung(tk);
                }
            }

            var f1 = this.Owner as form_ThongTin;
            f1.dataGridView.DataSource = AccountDAO.Instance.GetCurrentRecords(f1.CurrentPageIndex);
