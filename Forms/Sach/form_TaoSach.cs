...
namespace QLS
{
    public partial class form_TaoSach : Form
    {
        ...
        private void form_TaoSach_Load(object sender, EventArgs e)
        {
        ...
        private bool ThemSach()
        {
            ...
            data = SachDAO.Instance.ThemSach(maSach, tenSach, tacGia, theLoai, giaMua, giaBia, nhaXuatBan, namXuatBan, avatar);
        ...
        private bool KiemTra(List<Textbox_Custom> ls)
        {
            ...
            string masach = textbox_MaSach.Texts;
            if(SachDAO.Instance.KiemTraTrung(masach))
            {
                textbox_MaSach.BorderFocusColor = CaiDatDAO.Instance.color_Text_Red;
                textbox_MaSach.BorderColor = CaiDatDAO.Instance.color_Text_Red;
                textbox_MaSach.Focus();
                if (!check_ngonngu)
                {
                    label_Loi.Text = "Mã sách đã tồn tại";
                }
                else
                {
                    label_Loi.Text = "Book code already exists";
                }
                return false;
            }
