...
namespace QLS
{
    public partial class form_CapNhatAnh : Form
    {
        string tk;
        Image image;
        bool check_giaodien, check_ngonngu;
        public form_CapNhatAnh(string tk, Image image)
        {
            InitializeComponent();
        ...
        private void form_CapNhatAnh_Load(object sender, EventArgs e)
        {
            crop_PictureBox_Custom.Image = image;
        }
        ...
        private void button_DongY_Click(object sender, EventArgs e)
        {
            ...
            {
                if (item.GetType() == typeof(form_TaoSach))
                {
                    ((form_TaoSach)item).pictureBox_AnhDaiDien.Image = croppedImage;
                    break;
                }
                if (item.GetType() == typeof(form_ChinhSuaSach))
                {
                    ((form_ChinhSuaSach)item).pictureBox_AnhDaiDien.Image = croppedImage;
                    break;
                }
            }
            this.Close();
